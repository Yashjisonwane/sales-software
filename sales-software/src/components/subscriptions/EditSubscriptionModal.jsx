...
const EditSubscriptionModal = ({ isOpen, onClose, subscription, onSave, plans = [] }) => {
    ...
    useEffect(() => {
        if (subscription) {
            setForm({
                ...
                featureList: subscription.featureList || []
            });
        }
    }, [subscription]);
    ...
    const handleChange = (e) => {
        ...
        if (name === 'plan') {
            ...
            setForm(prev => ({ 
                ...prev, 
                plan: value, 
                amount: selectedPlan ? `$${selectedPlan.price}` : prev.amount,
                featureList: selectedPlan ? [...(selectedPlan.featureList || [])] : prev.featureList
            }));
        } else {
            ...
            <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                <div className="flex items-center justify-between px-8 pt-8 pb-2">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">{subscription.name}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-8 pb-8 pt-4 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Assigned Plan</label>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Monthly Amount</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Subscription Status</label>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-50">
                        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest">Active Entitlements</label>
                        ...
                        <div className="grid grid-cols-1 gap-2.5">
                            {form.featureList.length > 0 ? (
                                ...
                                <div className="py-8 text-center bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-100">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest italic">No entitlements found</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-6 sticky bottom-0 bg-white/80 backdrop-blur-sm">
                        ...
                        <button type="submit" className="flex-2 px-10 py-3.5 bg-blue-600 text-white rounded-2xl font-bold hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2 text-sm shadow-xl shadow-blue-100">
                            <Save size={18} /><span>Update Subscription</span>
                        </button>
                    </div>
                </form>
            </div>
