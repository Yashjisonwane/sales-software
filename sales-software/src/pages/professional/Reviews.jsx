...
const Reviews = () => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 lg:pb-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center relative overflow-hidden group">
                    ...
                    <h2 className="text-5xl md:text-7xl font-black text-gray-900 mb-2 leading-none tracking-tighter">{averageRating}</h2>
                ...
                <div className="lg:col-span-2 bg-white p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] shadow-sm border border-gray-100">
                    ...
                    <div className="space-y-4">
                        {ratingBreakdown.map((row) => (
                            <div key={row.stars} className="flex items-center gap-3 md:gap-4">
                                <div className="w-10 md:w-12 flex items-center gap-1">
                                    <span className="text-sm font-black text-gray-700">{row.stars}</span>
                                    <Star size={12} className="text-amber-400 fill-current" />
                                </div>
                                <div className="flex-1 h-2.5 md:h-3 bg-gray-50 rounded-full overflow-hidden border border-gray-100">
                                    <div
                                        className="h-full bg-gradient-to-r from-amber-400 to-orange-400 rounded-full transition-all duration-1000 delay-300"
                                        style={{ width: `${row.percentage}%` }}
                                    ></div>
                                </div>
                                <span className="w-10 md:w-12 text-right text-[10px] md:text-xs font-black text-gray-400 italic">{row.percentage}%</span>
                            </div>
                        ))}
                    </div>
