...
const extractLatLng = (source, latKeys = [], lngKeys = []) => {
    if (!source) return null;
    let lat = null;
    let lng = null;
    for (const key of latKeys) {
        const parsed = toNum(source?.[key]);
...
const getGoogleMapsCoordinateUrl = (lat, lng) =>
    `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${lat},${lng}`)}`;
const getGoogleMapsDirectionsUrl = (originLat, originLng, destLat, destLng) =>
    `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(`${originLat},${originLng}`)}&destination=${encodeURIComponent(`${destLat},${destLng}`)}&travelmode=driving`;
...
const TrackStatus = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(`${API_BASE_URL}/guest/track/${token}`);
                setData(res.data.data);
                const workerCoords = extractLatLng(
                    res.data?.data?.worker,
                    ['liveLat', 'lat', 'latitude'],
                    ['liveLng', 'lng', 'longitude']
                );
                if (workerCoords) {
                    console.log('[TrackStatus] API response professional coords:', workerCoords);
                    setLiveLocation((prev) => {
                        if (prev && prev.lat === workerCoords.lat && prev.lng === workerCoords.lng) return prev;
                        return workerCoords;
                    });
                }
                const customerCoords = extractLatLng(
                    res.data?.data,
                    ['customerLat', 'lat', 'latitude'],
                    ['customerLng', 'lng', 'longitude']
                );
                if (customerCoords) {
                    console.log('[TrackStatus] API response customer coords:', customerCoords);
                    setCustomerLocation((prev) => {
                        if (prev && prev.lat === customerCoords.lat && prev.lng === customerCoords.lng) return prev;
    ...
    useEffect(() => {
        ...
        const current = marker.getLngLat();
        const next = [liveLocation.lng, liveLocation.lat];
        console.log('[TrackStatus] Professional marker setLngLat [lng, lat]:', next);
        animateMarker(marker, [current.lng, current.lat], next);
        const now = Date.now();
        if (now - lastMoveAtRef.current > POLL_INTERVAL_MS) {
            map.easeTo({ center: next, duration: 800, essential: true });
    ...
    useEffect(() => {
        ...
        const next = [customerLocation.lng, customerLocation.lat];
        console.log('[TrackStatus] Customer marker setLngLat [lng, lat]:', next);
        if (!customerMarkerRef.current) {
            const el = document.createElement('div');
            el.innerHTML = `
                <div style="
                    width:20px;height:20px;border-radius:9999px;background:#2563eb;border:2px solid #fff;
                    box-shadow:0 6px 14px rgba(0,0,0,0.25);
                "></div>
            `;
            customerMarkerRef.current = new maplibregl.Marker({ element: el })
                .setLngLat(next)
                .setPopup(new maplibregl.Popup({ offset: 12 }).setHTML('<div style="font-size:12px"><b>Customer Location</b></div>'))
                .addTo(map);
        } else {
            customerMarkerRef.current.setLngLat(next);
        }
    }, [customerLocation]);
    ...
    useEffect(() => {
        ...
        const drawRoute = async () => {
            try {
                const routeCoords = {
                    from: [liveLocation.lng, liveLocation.lat],
                    to: [customerLocation.lng, customerLocation.lat]
                };
                console.log('[TrackStatus] OSRM route request coords [lng, lat]:', routeCoords);
                const routeRes = await fetch(
                    `https://router.project-osrm.org/route/v1/driving/${liveLocation.lng},${liveLocation.lat};${customerLocation.lng},${customerLocation.lat}?overview=full&geometries=geojson`
                );
                const routeJson = await routeRes.json();
                            ...
                            {liveLocation && (
                                <div className="mt-4 pt-4 border-t border-gray-100">
                                    ...
                                    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        {customerLocation && (
                                            <a
                                                href={getGoogleMapsCoordinateUrl(customerLocation.lat, customerLocation.lng)}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="bg-blue-50 border border-blue-100 rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-widest text-blue-700 text-center hover:bg-blue-100 transition-colors"
                                            >
                                        ...
                                        {liveLocation && (
                                            <a
                                                href={getGoogleMapsCoordinateUrl(liveLocation.lat, liveLocation.lng)}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="bg-purple-50 border border-purple-100 rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-widest text-purple-700 text-center hover:bg-purple-100 transition-colors"
                                            >
                                                Open Professional Live Location
                                            </a>
                                        )}
                                        {customerLocation && liveLocation && (
                                            <a
                                                href={getGoogleMapsDirectionsUrl(liveLocation.lat, liveLocation.lng, customerLocation.lat, customerLocation.lng)}
                                                target="_blank"
                                                rel="noreferrer"
                                                className="bg-green-50 border border-green-100 rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-widest text-green-700 text-center hover:bg-green-100 transition-colors sm:col-span-2"
                                            >
                                                Open Exact Directions
                                            </a>
                                        )}
                                    </div>
                                </div>
